    ...
    return ({
        ...
        ***************************************************************************/
        onErrorOutput: function (errMessage, invalidKeys) {
            var data = errMessage.split('.');
            var invalidValues = invalidKeys.map((ele, index) => ({
                key: ele,
                message: data[index]
            }));
        ...
        ***************************************************************************/
        inputErrorMessage: function (key, typeMessage) {
            let a = (typeMessage && typeMessage.length) ? /[aeiouAEIOU]/.test(typeMessage.charAt(0)) ? 'an' : 'a' : '';
            return `The '${key}' parameterss has an invalid input type` + (typeMessage ? `, it should be ${a} ${typeMessage}.` : '.');
        },
